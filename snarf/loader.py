...
def get_directory(pth):
    if not os.path.exists(pth):
        logger.debug('Creating directory {}', pth)
        os.makedirs(pth)
    return pth
...
class FilePath:
    ...
    def read(self):
        logger.debug('Reading from cache file: {}', self.filepath)
...
class Loader:
    ...
    def load_history(self, filename='history'):
        ...
        logger.debug('Reading history file: {}'.format(histfile))
        try:
            readline.read_history_file(histfile)
        except IOError as e:
            logger.debug('Error using `readline` history: {}'.format(e))
        else:
            logger.debug('History file at {} bytes'.format(readline.get_current_history_length()))
    ...
    def load_source(self, url, force_reload=False):
        ...
        if purl.scheme.lower() in ('file', ''):
            logger.debug('Reading from file: {}', purl.path)
            ...
            if not force_reload and cfp and cfp.exists():
                ...
                logger.debug('Retrieved {} bytes from {}', len(data), url)
